...
import { ClipboardList, PhoneCall, Handshake, Users, ShieldCheck, Map, Clock, CheckCircle, ChevronDown } from 'lucide-react';
...
const PROCESS_STEPS = [
  { num: '01', icon: ClipboardList, title: 'Isi Profil Kebutuhan', desc: 'Lengkapi varietas kopi yang diminati beserta estimasi volume bulanan/tahunan pada form.' },
  { num: '02', icon: PhoneCall, title: 'Konsultasi & Sampel', desc: 'Tim sales melakukan panggilan verifikasi kapasitas dan mengirimkan sampel uji cupping.' },
  { num: '03', icon: Handshake, title: 'Kontrak Kerjasama', desc: 'Persetujuan syarat MOQ, penawaran harga FOB/Lokal, penandatanganan jaminan pasokan.' },
];

const GUARANTEES = [
  { icon: Users, text: 'Mitra Petani Aktif: 1,200+ KK' },
  { icon: ShieldCheck, text: 'SNI / Organik / SCA Standard' },
  { icon: Map, text: 'Kebun Binaan: 4 Provinsi' },
  { icon: Clock, text: 'Respons dalam 24 Jam Kerja' },
];
            ...
            {PROCESS_STEPS.map((step, i) => (
              <Reveal key={i} delay={i * 0.12}>
                <div style={{ textAlign: 'center', padding: '16px 24px', position: 'relative', zIndex: 1 }}>
                  <div style={{ width: 80, height: 80, borderRadius: '50%', border: '2px solid var(--coffee-border-gold)', background: 'var(--coffee-bg-dark)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', color: 'var(--coffee-gold)', transition: 'border-color 0.3s', boxShadow: '0 4px 16px rgba(42,20,10,0.06)' }}>
                    <step.icon size={32} />
                  </div>
              ...
              <div style={{ position: 'sticky', top: '100px', alignSelf: 'start' }}>
                <div style={{ background: 'var(--coffee-surface)', border: '1px solid var(--coffee-border-gold)', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 16px 48px rgba(42,20,10,0.08)' }}>
                  ...
                  <div style={{ background: 'linear-gradient(135deg, #1C0F0A 0%, #2A140A 100%)', padding: '28px 28px 24px' }}>
                    <div style={{ width: 48, height: 48, borderRadius: '50%', border: '1px solid var(--coffee-border-gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12, color: 'var(--coffee-gold)', flexShrink: 0 }}>
                ...
                <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {[
                    { icon: Clock, text: 'Respons dalam 24 jam kerja' },
                    { icon: CheckCircle, text: 'Sampel gratis 250g per varietas' },
                    { icon: Handshake, text: 'Tanpa komitmen awal' },
                  ].map((t, i) => (
                    <motion.div
                      key={i}
                    ...
                    >
                      <span style={{ display: 'flex', color: 'var(--coffee-gold)' }}><t.icon size={18} /></span>
            ...
            {FAQS.map((faq, idx) => {
              ...
              return (
                <div key={idx} style={{ borderBottom: '1px solid var(--coffee-border-gold)' }}>
                  ...
                  >
                    <span>{faq.q}</span>
                    <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.3 }} style={{ display: 'flex', color: isOpen ? '#C5A880' : '#B0A49D' }}>
                      <ChevronDown size={20} />
                    </motion.div>
                  </motion.button>
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', flexWrap: 'wrap', gap: '24px' }}>
            {GUARANTEES.map((g, i) => (
              ...
              >
                <span style={{ display: 'flex' }}><g.icon size={20} /></span>
                {g.text}
              </motion.div>
            ))}
          </div>
